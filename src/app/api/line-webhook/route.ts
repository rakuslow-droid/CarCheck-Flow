          ...
          try {
            ...
            if (aiResult.inspectionDate) {
              ...
              if (!merchantSnap.empty) {
                ...
                await replyToLine(replyToken, `車検日（${aiResult.inspectionDate}）を登録しました！`, CHANNEL_ACCESS_TOKEN);
              }
            } else {
              await replyToLine(replyToken, "画像を解析しましたが、車検日が見つかりませんでした。", CHANNEL_ACCESS_TOKEN);
            }
          } catch (e: any) {
            console.error('Processing Error:', e.message);
            await replyToLine(replyToken, "申し訳ありません、画像の処理中にエラーが発生しました。", CHANNEL_ACCESS_TOKEN);
          }
        } 
        // テキストが送られてきた場合（テスト用）
        else if (event.message.type === 'text') {
          await replyToLine(replyToken, `「${event.message.text}」ですね！車検ステッカーの写真を送ってください。`, CHANNEL_ACCESS_TOKEN);
        }
      }
    }
